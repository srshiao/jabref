 ...
 */
public class NavigateAction extends SimpleCommand {
    ...
    public NavigateAction(StandardActions action, JabRefFrame frame, StateManager stateManager) {
        this.action = action;
        this.frame = frame;
    ...
    public void execute() {
        ...
        switch (action) {
            case BACK -> LOGGER.info("BACK ACTION submited");
            case FORWARD -> LOGGER.info("FORWARD ACTION submited");
            /*case CUT -> textInput.cut();
            case PASTE -> textInput.paste();
            case DELETE_ENTRY -> textInput.deleteNextChar();*/
            default -> throw new IllegalStateException("Only back/forward supported in Navigate Actions but got " + action);
        }
        ...
        stateManager.getFocusOwner().ifPresent(focusOwner -> {
            ...
            if (focusOwner instanceof TextInputControl) {
                ...
                switch (action) {
                    case BACK -> LOGGER.debug("BACK ACTION submited");
                    case FORWARD -> LOGGER.debug("FORWARD ACTION submited");
                    case CUT -> textInput.cut();
                    case PASTE -> textInput.paste();
                    case DELETE_ENTRY -> textInput.deleteNextChar();
                    default -> throw new IllegalStateException("Only back/forward supported in Navigate Actions but got " + action);
